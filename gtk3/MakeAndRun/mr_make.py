...
class MakefileManager:
    ...
    def makefile_candidates_from_src(self):
	    arqs = [ "Makefile", "makefile", \
	        os.path.join( "..", "Makefile" ), \
	        os.path.join( "..", "makefile" ) ]
	    
	    for i in range( len(arqs) ):
		    arqs[i] = os.path.abspath( os.path.join( self.src.get_dir(), arqs[i] ) )		
            return arqs
    ...
    def any_makefile_dir(self):
        for arq in self.makefile_candidates_from_src():
            if os.path.exists( arq ):
                return os.path.dirname( arq )
        return os.path.dirname( self.default_makefile_from_src() )
    ...
    def makefile_run(self, rebuilding):
        ...
        p.run_cmd_on_dir( cmd, self.any_makefile_dir() )
